...
export default function EditRecipeForm({ recipe }: { recipe: Recipe }) {
  ...
  const [ingredients, setIngredients] = useState(ingredientsToText(recipe.ingredie"use client";

import { useState } from "react";
import { useRouter } from "next/navigation";

const ALL_TAGS = [
  "meat","poultry","seafood","vegetarian","vegan",
  "pasta","rice","soup","curry","salad",
  "dessert","baking","breakfast","snack","sauce",
];
    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div>
        ...
        <input required type="text" value={name} onChange={e => setName(e.target.value)}
          className={inputClass} style={borderStyle} placeholder="e.g. Chicken & Zucchini Fritters" />
      </div>

      {/* Tags */}
      <div>
        <label className="text-[10px] font-bold uppercase tracking-wider block mb-2" style={{ color: "#D4A017" }}>Tags</label>
          ...
          {ALL_TAGS.map(tag => {
            ...
            return (
              <button key={tag} type="button" onClick={() => toggleTag(tag)}
                ...
                style={{ background: active ? "var(--mk-terracotta)" : "white", color: active ? "white" : "var(--mk-terracotta)", borderColor: active ? "var(--mk-terracotta)" : "var(--mk-border)" }}>
      ...
      {/* Story */}
      <div>
        <label className="text-[10px] font-bold uppercase tracking-wider block mb-1.5" style={{ color: "#D4A017" }}>Story / note</label>
        <textarea rows={3} value={story} onChange={e => setStory(e.target.value)}
          className={inputClass} style={borderStyle} placeholder="Where did you find this? What makes it special?" />
      </div>
      ...
      <div>
        ...
        <textarea rows={5} value={ingredients} onChange={e => setIngredients(e.target.value)}
          className={inputClass} style={borderStyle} placeholder={"2 chicken breasts\n1 zucchini\n1 egg\n…"} />
      </div>

      {/* Source URL */}
      <div>
        <label className="text-[10px] font-bold uppercase tracking-wider block mb-1.5" style={{ color: "#D4A017" }}>Source URL</label>
        <input type="url" value={sourceUrl} onChange={e => setSourceUrl(e.target.value)}
          className={inputClass} style={borderStyle} placeholder="https://www.instagram.com/reel/…" />
      </div>
      ...
      <div className="mt-6 pt-4 border-t" style={{ borderColor: "var(--mk-border)" }}>
        {deleting === "idle" && (
          <button type="button" onClick={() => setDeleting("confirm")}
          ...
          <div className="text-center">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setDeleting("idle")}
                ...
                style={{ borderColor: "var(--mk-border)", color: "#888" }}>Cancel</button>
              <button type="button"
                onClick={async () => {
                  setDeleting("working");
        ...
        {deleting === "working" && <p className="text-xs text-center text-neutral-400">Deleting…</p>}
      </div>
    </form>
  );
}
