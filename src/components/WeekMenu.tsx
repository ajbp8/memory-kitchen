...
export default function WeekMenu({
  ...
  const [dayTagFilter, setDayTagFilter] = useState<string | null>(null);
  // Recipe Genie
  ...
  const daySearchResults = useMemo(() => {
    ...
    return recipes.filter(r => {
      const matchText = q ? r.name.toLowerCase().includes(q) : true;
      const matchTag = dayTagFilter ? matchesFilter(r.cuisine_tags, dayTagFilter) : true;
      return matchText && matchTag;
    }).slice(0, 30);
  }, [daySearch, dayTagFilter, recipes]); // eslint-disable-line react-hooks/exhaustive-deps
          ...
          <div className="bg-white rounded-t-2xl w-full flex flex-col"
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4">
              ...
              <input type="text" value={daySearch} onChange={e => setDaySearch(e.target.value)}
                placeholder="Search recipes…" className="w-full rounded-xl px-3 py-2.5 text-sm border outline-none mb-2"
                style={{ borderColor: "var(--mk-border)", background: "white" }} />
              <div className="flex gap-1.5 overflow-x-auto pb-2" style={{ scrollbarWidth: "none" }}>
                <button onClick={() => setDayTagFilter(null)}
                  className="flex-shrink-0 text-[11px] font-bold px-2.5 py-1 rounded-full"
                  style={{ background: !dayTagFilter ? "var(--mk-terracotta)" : "rgba(212,160,23,0.13)", color: !dayTagFilter ? "white" : "var(--mk-terracotta)" }}>All</button>
                {CATEGORY_FILTERS.map(cat => (
                  <button key={cat.key} onClick={() => setDayTagFilter(prev => prev === cat.key ? null : cat.key)}
                    className="flex-shrink-0 text-[11px] font-bold px-2.5 py-1 rounded-full whitespace-nowrap"
                    style={{ background: dayTagFilter === cat.key ? "var(--mk-terracotta)" : "rgba(212,160,23,0.13)", color: dayTagFilter === cat.key ? "white" : "var(--mk-terracotta)" }}>
                    {cat.emoji} {cat.label}
                  </button>
                ))}
              </div>
